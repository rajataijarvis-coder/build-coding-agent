# Day 1, Tutorial 9-11: Setting Up the Project, Agent Skeleton & Architecture

**Course:** Build Your Own Coding Agent  
**Day:** 1  
**Tutorial:** 9-11 of 288 (consolidated)  
...
### Step 7: Extract Tools to builtins.py

First, let's move the built-in tools from `agent.py` to their own module so they're organized and easy to extend:
...
Update `tools/__init__.py` to export the built-in tools:
...
### Step 8: Update the Agent to Use Configuration

Now let's update our Agent to import tools from the new module:
...
# Import from submodules (Tutorial 8 package structure)
...
class Agent:
    ...
    - Tools from tools/builtins.py (modular organization)
    """
    ...
    def __init__(self, llm_strategy: Optional[LLMStrategy] = None):
        ...
        # Tools with registry - now from tools/builtins.py
        self._tools = ToolRegistry()
        self._setup_tools()
        
        # Command history (Tutorial 8 Command pattern)
    ...
    def _setup_tools(self) -> None:
        """Register built-in tools from tools/builtins.py."""
    ...
    def _handle_command(self, command: str) -> str:
        """Handle slash commands using Command pattern (Tutorial 8)."""
        ...
        # Command pattern: Wrap tool for tracking
        ...
        try:
            ...
            # Track in CommandHistory (Tutorial 8)
...
- ✅ **Tools organized in builtins.py** - all built-in tools now live in one place, easy to extend
...
- ✅ **Fallback to Ollama** keeps agent working even without API keys
...
git add src/coding_agent/tools/builtins.py
...
- Move built-in tools to tools/builtins.py for modularity
- Update Agent to import tools from tools module
