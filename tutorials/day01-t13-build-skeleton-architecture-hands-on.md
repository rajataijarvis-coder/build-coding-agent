...
### 🔄 Evolution Note: Command Pattern + Validation

This tutorial **evolves** the Command pattern from Tutorial 8 by adding **input validation**:

| Tutorial 8 | Tutorial 13 (This One) |
|------------|------------------------|
| `ToolCommand` wraps execution | `ToolCommand` + `validate_input()` |
| Tracks execution time | Tracks time **+ validates first** |
| Basic error handling | Structured error handling with `ToolError` |

The pattern stays the same, we just **add capabilities** on top. This is proper refactoring - foundation remains, features grow.
...
│       ├── config.py         # Configuration (from Tutorial 9)
│       ├── llm/
│       │   ├── __init__.py
│       │   ├── client.py     # LLMClient interface
│       │   └── factory.py    # LLM factory
│       ├── tools/
│       │   ├── __init__.py   # Tool exports (Tutorial 9)
│       │   ├── base.py       # Base tool class
│       │   ├── builtins.py   # Built-in tools (Tutorial 9)
...
│       ├── di/
│       │   ├── __init__.py
│       │   └── container.py  # DI Container (Tutorial 12)
...
```python
"""
Configuration management - Uses Tutorial 9's config structure.

See Tutorial 9-T11 for full implementation:
- AgentConfig: Root with llm, tools, logging sections
- LLMConfig: Provider settings (default_provider, claude_model, etc.)
- ToolConfig: Safety settings (allowed_directories, read_only, etc.)
- LoggingConfig: Log levels and verbose_events
...
@dataclass
class LLMConfig:
    """LLM provider configuration (from Tutorial 9)."""
    
    # Provider choice (Tutorial 9 uses default_provider)
    default_provider: str = "claude"
    
    # Model names per provider (Tutorial 9 structure)
    ...
    def has_anthropic_key(self) -> bool:
        return bool(self.anthropic_api_key and 
                   self.anthropic_api_key.startswith("sk-ant-"))
    ...
    def available_providers(self) -> List[str]:
        ...
        providers.append("ollama")  # Ollama always available if running
        return providers


@dataclass
class ToolConfig:
    """Tool configuration (from Tutorial 9)."""
...
@dataclass
class LoggingConfig:
    """Logging configuration (from Tutorial 9)."""
...
@dataclass
class AgentConfig:
    """Main agent configuration (from Tutorial 9-T11)."""
...
**Why use Tutorial 9's config structure:**
- ✅ **Already built** - No need to redefine
- ✅ **Working** - Tested and proven
- ✅ **Consistent** - Same across T9, T10, T11, T12
...
class ToolRegistry:
    ...
    def execute(self, name: str, **params: Any) -> ToolResult:
        """
        Execute a tool by name using Command pattern (Tutorial 8 + validation).
        
        This wraps tool execution in a Command for:
        - Execution tracking and timing
        - Input validation (Tutorial 13 evolution)
        - Error handling
        ...
        """
        from coding_agent.tools.command import ToolCommand, CommandResult
        from coding_agent.tools.base import ToolResult as BaseToolResult
        
        tool = self.get(name)
        
        # Validate input before creating command (Tutorial 13 evolution)
        try:
            tool.validate_input(params)
        except Exception as e:
            logger.error(f"Tool validation failed: {name} - {e}")
            raise ToolError(f"Validation failed for {name}: {e}") from e
        
        # Command pattern: Wrap execution for tracking (Tutorial 8)
        tool_cmd = ToolCommand(tool, params)
        
        if not tool_cmd.can_execute():
            raise ToolError(f"Tool cannot execute: {name}")
        
        try:
            result_str = tool_cmd.execute()
            
            # Create ToolResult from command execution
            ...
            logger.debug(
                f"Executed tool {name}: success={result.success}, "
                f"time={tool_cmd.execution_time:.2f}ms"
            )
            return result
...
### File 9: `src/coding_agent/tools/__init__.py`
...
**Why this file matters:**
- Provides clean imports: `from coding_agent.tools import HelpTool`
- Exports public API via `__all__`
- Centralizes tool access

---

### File 10: `src/coding_agent/tools/builtins.py`

```python
"""
Built-in tools - Core agent capabilities.

These tools come with every agent and provide basic functionality:
- help: Show available commands
- time: Show current time
- history: Show conversation history
- clear: Clear conversation history

From Tutorial 9: These were extracted from agent.py to make them
...
class HelpTool(BaseTool):
    ...
    This tool queries the ToolRegistry to get the list of
    registered tools and formats them for display.
    """
    
    def __init__(self, registry, config: Optional[Dict[str, Any]] = None):
        """
        Initialize with reference to the tool registry.
        
        Args:
            registry: ToolRegistry instance to query for tools
    ...
    def execute(self, **params) -> ToolResult:
        """Return formatted help text with all available tools."""
        try:
            ...
            lines.extend([
                "",
                "Usage:",
                "  Type /command to run a tool",
                "  Type normal text to chat with the agent"
            ])
            ...
            logger.error(f"Help tool failed: {e}")
            return ToolResult(
                success=False,
                content="Failed to retrieve help",
                error=str(e)
            )
...
class TimeTool(BaseTool):
    ...
    @property
    def description(self) -> str:
        return "Show current date and time"
...
class HistoryTool(BaseTool):
    ...
    Requires reference to the conversation manager to access history.
    """
    
    def __init__(self, context_manager, config: Optional[Dict[str, Any]] = None):
        """
        Initialize with reference to context manager.
        
        Args:
            context_manager: ConversationManager instance
            config: Optional configuration
        """
        super().__init__(config)
    ...
    def input_schema(self) -> Dict[str, Any]:
        return {
            ...
            "properties": {
                "limit": {
                    "type": "integer",
                    "description": "Maximum messages to show (default: all)",
                    "default": 10
                }
            },
            "required": []
    ...
    def execute(self, **params) -> ToolResult:
        ...
        try:
            ...
            if hasattr(self._context, 'get_messages'):
                messages = self._context.get_messages()
            else:
                messages = getattr(self._context, 'messages', [])
            ...
            return ToolResult(
                success=False,
                content="Failed to retrieve history",
                error=str(e)
            )
...
class ClearTool(BaseTool):
    ...
    Useful when starting a new task or when context gets too long.
    """
    ...
    def execute(self, **params) -> ToolResult:
        ...
        try:
            ...
            logger.error(f"Clear tool failed: {e}")
            return ToolResult(
                success=False,
                content="Failed to clear history",
                error=str(e)
            )
```

**Why built-in tools matter:**
- **help** - Self-documentation, users know what they can do
- **time** - Useful for timestamping or time-related queries
- **history** - Review what was said, debug context issues
- **clear** - Start fresh when conversation gets too long

These are the foundation - Day 3+ tutorials add file tools, shell tools, etc.

---

### File 11: `src/coding_agent/context/manager.py`
...
from coding_agent.tools.command import (
    ToolCommand, CommandResult, CommandHistory
)  # Command pattern from Tutorial 8
...
class Agent:
    ...
    def _setup_builtin_tools(self) -> None:
        ...
        - clear: Clear conversation history
        """
        # Import built-in tools from Tutorial 9 structure
...
Create a custom tool that echoes text back. First, import the built-in tools from Tutorial 9:
...
agent = Agent(config)
agent._setup_builtin_tools()  # Register built-in tools first (Tutorial 9)

# Then register your custom tool
